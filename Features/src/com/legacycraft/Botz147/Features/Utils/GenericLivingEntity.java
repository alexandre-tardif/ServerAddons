...
import org.getspout.spoutapi.gui.WidgetAnchor;
 ...
 public class GenericLivingEntity extends GenericContainer {
		 ...
		 this.addChildren(
				 _bar = (Container) new GenericContainer(// Used for the bar, this.children with an index 1+ are targets
						 ...
						 new GenericContainer( //
								 ...
								 _mana = (Gradient) new GenericGradient(new Color(0, 0, 1f, 0.75f)).setWidth(def_width) //
								 ).setMargin(1) //
	 ...
	 public GenericLivingEntity setEntity(String name, String prefix) {
		 ...
		 target = null;
		 setLabel((!"".equals(prefix) ? prefix : "") + Features.getColor(screen != null ? screen.getPlayer() : null, null) + name);
		 setFace("~" + name);
		 return this;
		 }	
	 ...
	 public GenericLivingEntity setEntity(LivingEntity entity, String prefix) {
		 if (entity != null && entity instanceof LivingEntity) {
			 target = entity;
			 setLabel((!"".equals(prefix) ? prefix : "") + Features.getColor(screen != null ? screen.getPlayer() : null, entity) + Features.getSimpleName(entity, !(getContainer() instanceof GenericLivingEntity)));
			 setFace(entity instanceof Player ? ((Player) entity).getName() : "+" + Features.getSimpleName(entity, false).replaceAll(" ", ""));
			 } else {
				 setEntity();
				 }		
		 return this;
	 ...
	 public int getTopValue() {
		 if (target != null) {
			 return Features.getHealth(target); // Needs a maxHealth() check
			 }
		 return 0;
		 }
	 ...
	 public int getBottomValue() {
		 if (target != null) {
			 return Features.getArmor(target);
			 }
		 return 0;
		 }
	 
	 /**
	  * Get the current mana bar value as a percentage.
	  * @return current mana
	  */
	 ...
	 public int getManaValue() {
		 if (target != null) {
			 if (target instanceof Player){
			     ...
			     return Features.getMana(player);
			     }
			 return 0;
			 }
		 return 0;
	 ...
	 /**	 
	  * Set the mana bar colour to use.	 
	  * @param color the solid colour for the health bar	 
	  * @return this	 
	  */
	 
      public GenericLivingEntity setManaColor(Color color) {
		 _mana.setColor(color);
		 return this;
		 }
	 ...
	 @Override	public void onTick() {
		 ...
		 _mana.setWidth((box.getWidth() * getManaValue()) / 100);
		 }
	 }
